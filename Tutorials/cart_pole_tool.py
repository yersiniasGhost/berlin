# Cart Pole Deep Q Learning Networks.

''' DQN- Lets us modify the q learning algo so that we do not need to store all the action
...
class DeepQLearning:
    ...
    '''For this function we define a index where we will randomly explore if the current episode
    is below a certain number... randomNumber samples a number between 0-1, if the number
    is smaller then episolon then we randomly choose action 1 or 0. If not then we use
    the greedy approach; use a nueral network to predict the action value functions, we obtain
    Q(S,A=0), Q(S,A=1), we choose the larger action to choose for A (0,1). Good to slower lowly
    epsilon as episodes go on.'''
    ...
    '''Will run the steps within an episode until the terminal state is reached true
    hence the pole falls or reaches time limit. Once that happens it starts another
    episode with a reset environment. '''
