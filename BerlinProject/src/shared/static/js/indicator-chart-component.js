...
class IndicatorChartComponent {
    ...
    init() {
        console.log('=== INDICATOR CHART COMPONENT INIT ===');
        console.log('Looking for container ID:', this.containerId);
        ...
        console.log('Proceeding with component initialization...');
    ...
    loadIndicators(monitorConfig) {
        ...
        console.log('Monitor config indicators:', monitorConfig.indicators);
        ...
        monitorConfig.indicators.forEach(indicator => {
            ...
            console.log(`Indicator ${indicator.name}: function=${indicator.function} -> displayType=${type}`);
        ...
        console.log('Final indicators object:', this.indicators);
        console.log('=== END LOADING INDICATORS DEBUG ===');
    ...
    toggleIndicator(indicatorName, enabled) {
        console.log(`=== TOGGLE INDICATOR DEBUG ===`);
        console.log(`Toggling ${indicatorName}: ${enabled}`);
        ...
        console.log('Enabled indicators:', Array.from(this.enabledIndicators));
        console.log('Indicator details:', this.indicators[indicatorName]);
    ...
    setChartData(chartData) {
        console.log('=== SET CHART DATA DEBUG ===');
        ...
        console.log('enabledIndicators:', Array.from(this.enabledIndicators));
     ...
     */
    createMainChart() {
        console.log('=== CREATE MAIN CHART DEBUG ===');
        console.log('Container ID:', `${this.containerId}_main_chart`);
        
        const container = document.getElementById(`${this.containerId}_main_chart`);
        console.log('Container found:', !!container);
        console.log('Candlestick data available:', !!this.chartData.candlestick_data);
        console.log('Candlestick data length:', this.chartData.candlestick_data?.length || 0);
        
        if (!container || !this.chartData.candlestick_data) {
            console.warn('Exiting createMainChart early - missing container or data');
            return;
        }
        
        console.log('Proceeding with main chart creation...');
        ...
        // SMA indicators are now created as separate charts, not overlays
            ...
            chart: {
                ...
                events: {
                    selection: function(event) {
                        if (event.xAxis && typeof syncAllCharts === 'function') {
                            const extremes = event.xAxis[0];
                            syncAllCharts(extremes.min, extremes.max, 'indicator_main');
                        }
                    }
                }
            },
            ...
            xAxis: {
                ...
                events: {
                    setExtremes: function(e) {
                        if (e.trigger !== 'syncExtremes' && typeof syncAllCharts === 'function') {
                            syncAllCharts(e.min, e.max, 'indicator_main');
                        }
                    }
                }
            },
    ...
    createIndicatorCharts() {
        ...
        this.enabledIndicators.forEach(indicatorName => {
            ...
            if (indicator.displayType === 'macd' || indicator.displayType === 'rsi' || indicator.displayType === 'sma') {
    ...
    createIndividualIndicatorChart(indicatorName, containerId, type) {
        ...
        if (type === 'macd') {
            // MACD with separated layout: MACD/Signal lines above, histogram below
            ...
            if (macdData.macd.length > 0 && macdData.signal.length > 0) {
                series = [
                    ...
                    },
                    {
                        ...
                        color: '#F18F01',
                        pointWidth: 2,
                        yAxis: 1  // Bottom section
                    }
                ];
                
                // Don't show trigger points on MACD chart - they should be in separate trigger chart
            } else {
            ...
            // Configure dual y-axis like your indicator_visualization
            yAxisConfig = [
                {
            ...
            // SMA as separate chart (like MACD)
            const smaData = this.prepareSMAData(indicatorName);
            
            console.log(`SMA data for ${indicatorName}:`, {
                smaPoints: smaData.length,
            ...
            if (smaData.length > 0) {
                ...
                console.warn(`No SMA data available for ${indicatorName}`);
                series = [{
                    name: 'No SMA Data Available',
                    data: [],
                    type: 'line'
                }];
            }
        ...
        const config = {
            chart: {
                ...
                events: {
                    selection: function(event) {
                        if (event.xAxis && typeof syncAllCharts === 'function') {
                            const extremes = event.xAxis[0];
                            syncAllCharts(extremes.min, extremes.max, indicatorName);
                        }
                    }
                }
            },
            ...
            xAxis: {
                ...
                events: {
                    setExtremes: function(e) {
                        if (e.trigger !== 'syncExtremes' && typeof syncAllCharts === 'function') {
                            syncAllCharts(e.min, e.max, indicatorName);
                        }
                    }
                }
            },
    ...
    /**
     * Create individual trigger chart showing time-decayed values as line chart
     */
    createTriggerChart(indicatorName) {
        ...
        // Get time-decayed trigger data (not just 0-1 values)
        const triggerData = this.prepareTriggerData(indicatorName);
        
        console.log(`Creating trigger chart for ${indicatorName} with ${triggerData.length} points`);
        console.log(`Sample trigger data:`, triggerData.slice(0, 5));
        ...
        const config = {
            chart: {
                ...
                events: {
                    selection: function(event) {
                        if (event.xAxis && typeof syncAllCharts === 'function') {
                            const extremes = event.xAxis[0];
                            syncAllCharts(extremes.min, extremes.max, `${indicatorName}_trigger`);
                        }
                    }
                }
            },
            title: { text: `${indicatorName} Time-Decayed Triggers` },
            xAxis: {
                type: 'datetime',
                events: {
                    setExtremes: function(e) {
                        if (e.trigger !== 'syncExtremes' && typeof syncAllCharts === 'function') {
                            syncAllCharts(e.min, e.max, `${indicatorName}_trigger`);
                        }
                    }
                }
            },
            yAxis: {
                title: { text: 'Time-Decayed Value' },
                plotLines: [{
                    value: 0,
                    color: '#999',
                    width: 1,
            ...
            series: [{
                name: `${indicatorName} Decayed`,
                data: triggerData,
                color: '#FF6B35',
                lineWidth: 2,
                tooltip: {
                    pointFormat: '<b>Decayed Value:</b> {point.y:.4f}'
                }
            }],
            credits: { enabled: false }
        };
    ...
    prepareSMAData(indicatorName) {
        // First try to get SMA from component_data (preferred for replay_visualization)
        if (this.chartData.component_data) {
            const smaKey = `${indicatorName}_sma`;
            const smaComponent = this.chartData.component_data[smaKey];
            
            console.log(`SMA component data for ${indicatorName}:`, {
                smaKey,
                hasSma: !!smaComponent,
                componentDataKeys: Object.keys(this.chartData.component_data),
                smaComponent: smaComponent,
                sampleData: smaComponent?.data?.slice(0, 3)
            });
            
            if (smaComponent && smaComponent.data) {
                console.log(`Found SMA component data with ${smaComponent.data.length} points`);
                return smaComponent.data;
            } else {
                console.warn(`SMA component ${smaKey} not found or has no data`);
            }
        } else {
            console.warn(`No component_data available for SMA ${indicatorName}`);
        }
        
        // Fallback: Try multiple data sources for SMA values
    ...
    /**
     * Prepare MACD data based on component data or indicator history
     */
    prepareMACDData(indicatorName) {
        // First try to get MACD components from component_data (preferred for replay_visualization)
        if (this.chartData.component_data) {
            const macdKey = `${indicatorName}_macd`;
            const signalKey = `${indicatorName}_signal`;
            const histogramKey = `${indicatorName}_histogram`;
            
            const macdComponent = this.chartData.component_data[macdKey];
            const signalComponent = this.chartData.component_data[signalKey];
            const histogramComponent = this.chartData.component_data[histogramKey];
            
            console.log(`MACD component data for ${indicatorName}:`, {
                macdKey, signalKey, histogramKey,
                hasMacd: !!macdComponent,
                hasSignal: !!signalComponent,
                hasHistogram: !!histogramComponent
            });
            
            if (macdComponent && signalComponent && histogramComponent) {
                return {
                    macd: macdComponent.data || [],
                    signal: signalComponent.data || [],
                    histogram: histogramComponent.data || []
                };
            }
        }
        
        // Fallback: Try to get MACD data from indicator_history
        if (this.chartData.indicator_history) {
