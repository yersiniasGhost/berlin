...
class IndicatorChartComponent {
    ...
    updateCharts() {
        ...
        // Create separate trigger charts like indicator_visualization
    ...
    createIndividualIndicatorChart(indicatorName, containerId, type) {
        ...
        if (type === 'macd') {
            // MACD with separated layout: MACD/Signal lines above, histogram below (your pattern)
            ...
            if (macdData.macd.length > 0 && macdData.signal.length > 0) {
                ...
                // Add trigger scatter points if available
                if (this.triggerData && this.triggerData[indicatorName]) {
                    const triggerPoints = [];
                    this.triggerData[indicatorName].forEach((point, index) => {
                        const [timestamp, triggerValue] = point;
                        if (triggerValue === 1 && macdData.macd[index]) {
    ...
    /**
     * Create individual trigger chart - Simple scatter points like your indicator_visualization
     */
    createTriggerChart(indicatorName) {
        ...
        // Your pattern: Only show trigger points where value === 1, as simple scatter
        const triggerData = this.prepareTriggerData(indicatorName);
        const triggerPoints = [];
        
        triggerData.forEach(([timestamp, triggerValue]) => {
            if (triggerValue === 1) {
                triggerPoints.push([timestamp, 1]);
            }
        });
        ...
        const config = {
            ...
            title: { text: `${indicatorName} Triggers (0-1)` },
            xAxis: {
                type: 'datetime',
                events: {
                    setExtremes: function(e) {
            ...
            yAxis: {
                ...
                labels: {
                    formatter: function() {
                        return this.value === 0 ? '0' : this.value === 1 ? '1' : this.value.toFixed(1);
                    }
                }
            },
            series: [{
                name: `${indicatorName} Triggers`,
                data: triggerData, // Use full trigger data, not just points where value === 1
                type: 'area',
                color: '#45B7D1', // Light blue like your indicator_visualization
                fillOpacity: 0.3,
                step: 'left', // Step-like appearance for 0-1 jumps
                tooltip: {
                    pointFormat: '<b>Trigger:</b> {point.y}'
                }
            }],
            credits: { enabled: false },
            plotOptions: {
