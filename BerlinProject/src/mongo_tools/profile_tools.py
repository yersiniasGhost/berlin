...
from config.types import PYMONGO_ID, PROFILE_COLLECTION, PyObjectId
...
class ProfileTools:
    ...
    @classmethod
    def create_profile_from_json_file(cls, json_file_path: str) -> Optional[str]:
        try:
    ...
    def get_profile(cls, profile_id: PYMONGO_ID) -> Profile:
        data = cls.get_collection().find_one({"_id": PyObjectId(profile_id)})
