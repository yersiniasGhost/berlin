...
from config import BUY, SELL, HOLD, ACTION
...
class SimplePosition(State):
    ...
    def update_and_calculate_reward(self, action: str, tick: TickData) -> float:
        ...
        if not self.is_action_valid(action):
            return 0.0  # Return 0 reward for invalid actions

        if action == BUY:
            self.position += self.lot_size
            self.buy_price = tick.close
            self.cash -= self.buy_price * self.lot_size
            self.in_position_steps = 0
        elif action == SELL:
            gain = (tick.close - self.buy_price) * self.position
            step_reward += gain
        ...
        if self.position > 0:
            ...
            # step_reward += target
            if self.in_position_steps > HOLD_TIME_LIMIT:
                step_reward -= 0.0 * (self.in_position_steps - HOLD_TIME_LIMIT)
