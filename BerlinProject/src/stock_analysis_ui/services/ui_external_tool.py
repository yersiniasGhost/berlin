...
class UIExternalTool(ExternalTool):
    ...
    def clear_meaningful_data(self, card_id: str):
        """Clear meaningful data for a specific card"""
        try:
            # Clear data for specific card
            if card_id in self.last_meaningful_data:
                del self.last_meaningful_data[card_id]
            if card_id in self.last_update_time:
                del self.last_update_time[card_id]
            if card_id in self.update_counter:
                del self.update_counter[card_id]
            if card_id in self.failed_emit_counter:
                del self.failed_emit_counter[card_id]
                
            logger.debug(f"Cleared meaningful data for card {card_id}")
            
        except Exception as e:
            logger.error(f"Error clearing meaningful data for card {card_id}: {e}")
